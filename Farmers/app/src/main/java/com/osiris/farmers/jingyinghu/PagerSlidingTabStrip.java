 ...
 * 可以设置 tab 的类型为 textview 还是 icon。
 * 对于 textview 可设置字库属性。 通过提供方法如滑动指示器 下划线 tab 风格线 tab 权重等达到自定义的效果。
...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        tabMarginLeft = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsIndicatorMarginLeft, tabMarginLeft);
        tabMarginRight = a.getDimensionPixelSize(R.styleable.PagerSlidingTabStrip_pstsIndicatorMarginLeft, tabMarginRight);
...
//240+204
        ...
        if (position > 0 || offset > 0) {
            newScrollX -= scrollOffset;//600-156=444
        }
        //滑动到的位置。
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawRect(lineLeft + tabMarginLeft, height - indicatorHeight, lineRight-tabMarginRight, height, rectPaint);
    ...
    private class PageListener implements OnPageChangeListener {
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            currentPosition = position;//当前view的位置
            currentPositionOffset = positionOffset;//当前view滑动的距离
