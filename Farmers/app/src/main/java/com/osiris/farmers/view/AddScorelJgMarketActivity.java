...
import com.osiris.farmers.shichang.AddScoreShichangAdapter;
import com.osiris.farmers.utils.JsonUtils;
import com.osiris.farmers.view.adapter.AddScoreJgMraketAdapter;
...
public class AddScorelJgMarketActivity extends BaseActivity {
    ...
    private List<AddScore.PingjiaxxsBean> pingjiaxxs = new ArrayList<>();
    private AddScoreJgMraketAdapter marketScoreAdapter = new AddScoreJgMraketAdapter(pingjiaxxs);
    ...
    public void init() {
        ...
        xxz = getIntent().getIntExtra("xxz", 1);
        ...
        spinner_market.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                jyhid = dataList.get(position).getId();
            }
...
//					}
    ...
    private void commitData() {
        ...
        paramMap.put("id", String.valueOf(GlobalParams.id));
        	paramMap.put("type","1");
        paramMap.put("marketid", String.valueOf(jyhid));
        StringBuffer stringBuffer = new StringBuffer();
        for (AddScore.PingjiaxxsBean marketsBean : pingjiaxxs) {
        ...
        NetRequest.request(url, ApiRequestTag.DATA, paramMap, new NetRequestResultListener() {
            ...
            public void requestSuccess(int tag, String successResult) {
                ...
                new Handler().post(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(AddScorelJgMarketActivity.this, "全部提交成功", Toast.LENGTH_SHORT).show();
                        finish();
                    }
                });
...
//http://39.97.235.7:8086/wisdom/app/showgetZgpingjiaAdd.action
        ...
        NetRequest.request(url, ApiRequestTag.DATA, paramMap, new NetRequestResultListener() {
            ...
            public void requestSuccess(int tag, String successResult) {
                ...
                if (json.has("markets")){
                    ...
                    dataList.addAll(Arrays.asList(datas));
                    Log.d("zkf","dataList size" + dataList.size());

                    for (AddScore.MarketsBean marketsBean:dataList){
                        selectName.add("           " + marketsBean.getMarketnm() + "      ");

                    }
                    jyhid = dataList.get(0).getId();

                    marketAdapter.notifyDataSetChanged();
                }

                if (json.has("pingjiaxxs")){
                    AddScore.PingjiaxxsBean[] datas = JsonUtils.fromJson(json.get("pingjiaxxs").getAsJsonArray(),
                            AddScore.PingjiaxxsBean[].class);
                    pingjiaxxs.addAll(Arrays.asList(datas));
                    Log.d("zkf"," size" + pingjiaxxs.size());
                    marketScoreAdapter.notifyDataSetChanged();
